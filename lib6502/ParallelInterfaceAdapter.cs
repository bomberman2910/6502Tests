        ...
        public override void SetData(byte data, ushort address)
        {
            ...
            switch (address)
            {
                ...
                case var add when add == Start + 3: //RDYR (- - - - - - RDYB RDYA)
                    readyA = OutA switch
                    {
                        true when Cpu6502.CheckBit(data, 0) => true,
                        true when !Cpu6502.CheckBit(data, 0) => false,
                        _ => readyA
                    };
                    readyB = OutB switch
                    {
                        true when Cpu6502.CheckBit(data, 1) => true,
                        true when !Cpu6502.CheckBit(data, 1) => false,
                        _ => readyB
                    };
                    break;
            }
